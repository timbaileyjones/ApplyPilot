...
Do NOT add any tools, languages, or frameworks not listed above. Every skill you include must be in the SKILLS BOUNDARY. No exceptions.
...
1. Adding ANY tool, language, or framework not in the allowed list. The allowed skills are ONLY: {skills_str}. "Closely related" is not an excuse — if it's not in the list, it's a FAIL.
...
## TOLERANCE RULE:
- Reframing a metric with slightly different wording is acceptable.
- Only FAIL if there are lies: invented projects, fake companies, fake degrees, inflated numbers, or skills not in the allowed list.

Be strict about skills — if it's not in the allowed list, it's a FAIL regardless of how "related" it seems. Do not fail for style, tone, or restructuring."""
...
def run_tailoring(min_score: int = 7, limit: int = 2000,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(_tailor_one, job): job for job in jobs}
        for future in concurrent.futures.as_completed(futures):
            ...
            # Persist immediately so aborts don't lose progress (main-thread DB write)
